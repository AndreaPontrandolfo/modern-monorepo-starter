...
var loadJson = async (filepath) => {
  try {
    return _chunk4R4UQZRTjs.jsoncParse.call(void 0, await _fs2.default.promises.readFile(filepath, "utf8"));
  } catch (error) {
...
var colorize = (type, data, onlyImportant = false) => {
  ...
  return _chunkNK5HUMBWjs.colorette_exports[color](data);
};
var makeLabel = (name, input, type) => {
  return [
    name && `${_chunkNK5HUMBWjs.dim.call(void 0, "[")}${name.toUpperCase()}${_chunkNK5HUMBWjs.dim.call(void 0, "]")}`,
...
var reportSize = (logger, format, files) => {
  ...
  for (const name of filenames) {
    logger.success(format, `${_chunkNK5HUMBWjs.bold.call(void 0, padRight(name, maxLength))}${_chunkNK5HUMBWjs.green.call(void 0, prettyBytes(files[name]))}`);
  }
};
